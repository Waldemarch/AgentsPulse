...
- GitHub release notes (`gh release create vX.Y.Z dist/AgentsPulse.exe --title "vX.Y.Z" --notes "..."`) must use the exact content from the version's `CHANGELOG.md` section (the `### Added` / `### Changed` / `### Fixed` / `### Removed` blocks), followed by a `[Full changelog](compare-url)` link and a `[README for this version](https://github.com/Waldemarch/usage-monitor-for-Codex-and-codex/blob/vX.Y.Z/README.md)` link
