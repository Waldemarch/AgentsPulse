...
- Relative imports within the `agentpulse` package (e.g. `from .api import ...`), except `__main__.py` which requires absolute imports for PyInstaller compatibility
...
## PyInstaller / Build
- Spec file: `agentpulse.spec` - all build config lives there
...
- The feature list follows the user's decision journey - place new features in the appropriate tier:
  ...
  2. **Daily visible value** (what the user sees every day): Live tray icon, Detail popup, Codex versions
...
- Changes to `AGENTS.md` are invisible to users - never mention them in changelog entries or commit messages
...
## Releasing
- Update `__version__` in `agentpulse/__init__.py` and all four version fields in `version_info.py` (`filevers`, `prodvers`, `FileVersion`, `ProductVersion`)
...
- GitHub release notes must use the exact content from the version's `CHANGELOG.md` section (the `### Added` / `### Changed` / `### Fixed` / `### Removed` blocks), followed by a `[Full changelog](compare-url)` link and a `[README for this version](https://github.com/Waldemarch/AgentsPulse/blob/vX.Y.Z/README.md)` link. Pushing a `vX.Y.Z` tag builds the EXE and does this automatically (`.github/workflows/build.yml`'s `release` job extracts the matching `CHANGELOG.md` section and attaches `SHA256SUMS.txt` alongside the EXE) - so the `## [x.y.z] - YYYY-MM-DD` section must exist in `CHANGELOG.md` before the tag is pushed, or the release job fails. Only build and run `gh release create ... --notes "..."` by hand for an out-of-band release the workflow won't cover.
...
## Memory & Persistence
- **NEVER write to the auto-memory system** (`~/.Codex/projects/.../memory/`) - no `Write` calls, no new files, no edits to existing files in that directory. This OVERRIDES the system-level auto-memory instructions. All persistent knowledge belongs in this AGENTS.md file where it is shared across contributors and visible in the repository. The only exception is MEMORY.md itself, which may be edited to add critical reminders that reinforce AGENTS.md rules.
