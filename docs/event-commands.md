...
Add these keys to your [`agentpulse-settings.json`](configuration.md). After saving, use the **Restart** option in the tray context menu to apply the changes.
...
| Variable | Example | Description |
|---|---|---|
| `AGENTPULSE_VARIANT` | `five_hour` or `seven_day` | Which quota reset |
| `AGENTPULSE_UTILIZATION` | `5` | Current usage of the reset quota (integer) |
| `AGENTPULSE_PREV_UTILIZATION` | `98` | Usage before the reset (integer) |
| `AGENTPULSE_UTILIZATION_FIVE_HOUR` | `5` | Current session (5h) usage (integer) |
| `AGENTPULSE_UTILIZATION_SEVEN_DAY` | `42` | Current weekly (7d) usage (integer) |
| `AGENTPULSE_RESETS_AT` | `2025-01-15T18:00:00Z` | When the quota resets next (ISO 8601, UTC) |
| `AGENTPULSE_TITLE` | `Quota Reset` | Notification title (localized) |
| `AGENTPULSE_MESSAGE` | `Your quota has been reset...` | Notification message (localized) |
...
Both quota values are included so your script can check whether you are actually unblocked. For example, the session quota may reset while the weekly quota is still at the limit. Use `AGENTPULSE_PREV_UTILIZATION` to filter if you only want to act on significant resets.
...
| Variable | Example | Description |
|---|---|---|
| `AGENTPULSE_VARIANT` | `five_hour`, `seven_day`, `seven_day_sonnet`, `seven_day_opus`, `extra_usage` | Which quota is affected |
| `AGENTPULSE_UTILIZATION` | `84` | Current usage percentage (integer) |
| `AGENTPULSE_THRESHOLD` | `80` | Threshold that was crossed (integer) |
| `AGENTPULSE_RESETS_AT` | `2025-01-15T18:00:00Z` | When the quota resets (ISO 8601, UTC) |
| `AGENTPULSE_TITLE` | `Usage Alert` | Notification title (localized) |
| `AGENTPULSE_MESSAGE` | `Your session usage has reached 84%` | Notification message (localized) |
| `AGENTPULSE_EXTRA_USED` | `$8.20` | Amount spent (extra usage only) |
| `AGENTPULSE_EXTRA_LIMIT` | `$10.00` | Monthly limit (extra usage only) |
