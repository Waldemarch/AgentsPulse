...
The app itself never contacts GitHub - its built-in network traffic is limited to provider usage endpoints (`api.anthropic.com` for Claude, and `chatgpt.com` for Codex when a Codex CLI token is present). Update checking is available as an optional PowerShell script that you run via [event commands](event-commands.md). This keeps the security model explicit while giving you full control over when and how updates are checked.
...
Save the following as `check-update.ps1` next to your `AgentsPulse.exe` (or in the project root when running from source):

```powershell
$currentVersion = if ($env:USAGE_MONITOR_VERSION) { $env:USAGE_MONITOR_VERSION } else { '0.0.0' }

$releaseUrl = 'https://api.github.com/repos/Waldemarch/usage-monitor-for-claude-and-codex/releases/latest'
...
Add the script to your [`agentpulse-settings.json`](configuration.md). Choose when to check based on your preference:
...
2. The script sends a single HTTPS request to `https://api.github.com/repos/Waldemarch/usage-monitor-for-claude-and-codex/releases/latest`
