...
class TestLoadSettings(unittest.TestCase):
    ...
    def test_legacy_app_dir_file_loaded(self):
        """Legacy settings filename is still loaded as a fallback."""
        with TemporaryDirectory() as app_tmp, TemporaryDirectory() as home_tmp:
            settings = {'poll_interval': 240}
            legacy_name = settings_mod.LEGACY_SETTINGS_FILENAMES[0]
            (Path(app_tmp) / legacy_name).write_text(json.dumps(settings), encoding='utf-8')
            result = _load(Path(app_tmp), Path(home_tmp))
        self.assertEqual(result, settings)

    def test_new_settings_filename_wins_over_legacy(self):
        """Canonical AgentPulse settings file takes priority over legacy file."""
        with TemporaryDirectory() as app_tmp, TemporaryDirectory() as home_tmp:
            legacy_name = settings_mod.LEGACY_SETTINGS_FILENAMES[0]
            (Path(app_tmp) / legacy_name).write_text(json.dumps({'poll_interval': 240}), encoding='utf-8')
            (Path(app_tmp) / settings_mod.SETTINGS_FILENAME).write_text(json.dumps({'poll_interval': 300}), encoding='utf-8')
            result = _load(Path(app_tmp), Path(home_tmp))
        self.assertEqual(result['poll_interval'], 300)
...
class TestDashboardSettings(unittest.TestCase):
    """Tests for dashboard-editable settings persistence."""

    def test_save_dashboard_settings_writes_canonical_file(self):
        with TemporaryDirectory() as app_tmp, TemporaryDirectory() as home_tmp:
            fake_file = str(Path(app_tmp) / 'agentpulse' / 'settings.py')
            with patch.object(settings_mod, '__file__', fake_file), \
                 patch.object(Path, 'home', return_value=Path(home_tmp)):
            ...
            self.assertEqual(saved['quiet_hours_end'], '07:15')

    def test_save_dashboard_settings_rejects_unknown_key(self):
        with TemporaryDirectory() as app_tmp, TemporaryDirectory() as home_tmp:
            fake_file = str(Path(app_tmp) / 'agentpulse' / 'settings.py')
            with patch.object(settings_mod, '__file__', fake_file), \
                 patch.object(Path, 'home', return_value=Path(home_tmp)):
                ok, errors, _path = settings_mod.save_dashboard_settings({'api_token': 'secret'})

        self.assertFalse(ok)
        self.assertTrue(errors)

    def test_save_dashboard_settings_rejects_bad_time(self):
        with TemporaryDirectory() as app_tmp, TemporaryDirectory() as home_tmp:
            fake_file = str(Path(app_tmp) / 'agentpulse' / 'settings.py')
            with patch.object(settings_mod, '__file__', fake_file), \
                 patch.object(Path, 'home', return_value=Path(home_tmp)):
                ok, errors, _path = settings_mod.save_dashboard_settings({'quiet_hours_start': '25:99'})
