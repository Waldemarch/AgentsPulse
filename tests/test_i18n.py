...
from agentpulse.i18n import LOCALE_DIR, detect_lang_code, load_translations
...
@patch('agentpulse.i18n.locale.normalize', side_effect=_mock_normalize)
class TestDetectLangCode(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._patch_dir = patch('agentpulse.i18n.LOCALE_DIR', self._locale_dir)
...
class TestLoadTranslations(unittest.TestCase):
    ...
    @patch('agentpulse.settings.LANGUAGE', '')
    @patch('agentpulse.i18n.locale.normalize', side_effect=_mock_normalize)
    @patch('agentpulse.i18n.locale.getlocale', return_value=('de_DE', 'UTF-8'))
    def test_loads_detected_locale(self, _mock_get, _mock_norm):
        ...
        with TemporaryDirectory() as tmp:
            ...
            with patch('agentpulse.i18n.LOCALE_DIR', locale_dir):
    ...
    @patch('agentpulse.settings.LANGUAGE', '')
    @patch('agentpulse.i18n.locale.normalize', side_effect=_mock_normalize)
    @patch('agentpulse.i18n.locale.getlocale', return_value=(None, None))
    ...
    def test_language_setting_overrides_locale(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            with patch('agentpulse.settings.LANGUAGE', 'ja'), \
                 patch('agentpulse.i18n.LOCALE_DIR', locale_dir):
                result = load_translations()

        self.assertEqual(result['title'], 'Japanese')

    @patch('agentpulse.settings.LANGUAGE', 'xx')
    @patch('agentpulse.i18n.locale.normalize', side_effect=_mock_normalize)
    @patch('agentpulse.i18n.locale.getlocale', return_value=('de_DE', 'UTF-8'))
