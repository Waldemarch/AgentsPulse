...
from agentpulse.cache import CacheSnapshot, UpdateResult, UsageCache
...
class TestLockBehavior(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_usage', return_value=_SUCCESS_DATA)
    ...
    def test_lock_released_on_exception(self):
        ...
        cache = _make_cache()
        with patch('agentpulse.cache.fetch_usage', side_effect=RuntimeError('boom')):
    ...
    @patch('agentpulse.cache.fetch_usage', return_value=_AUTH_ERROR_DATA)
    @patch('agentpulse.cache.read_access_token', return_value='token-abc')
    def test_refreshing_reset_on_refresh_exception(self, _mock_token, _mock_fetch):
        ...
        cache = _make_cache()
        with patch('agentpulse.cache.refresh_token', side_effect=RuntimeError('cli crash')):
            with self.assertRaises(RuntimeError):
                cache.update()
...
class TestCooldownBehavior(unittest.TestCase):
    ...
    def test_first_call_always_proceeds(self):
        ...
        cache = _make_cache()
        with patch('agentpulse.cache.fetch_usage', return_value=_SUCCESS_DATA):
            result = cache.update()
        self.assertIsNotNone(result.data)

    @patch('agentpulse.cache.fetch_usage', return_value=_ERROR_DATA)
...
class TestSuccessState(unittest.TestCase):
    ...
    @patch('agentpulse.cache.read_access_token', return_value='new-token')
    @patch('agentpulse.cache.fetch_usage', return_value=_SUCCESS_DATA)
...
class TestErrorState(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_usage', return_value=_SERVER_MSG_DATA)
    def test_server_message_appended_to_last_error(self, _mock):
        cache = _make_cache()
        cache.update()
        self.assertEqual(cache.last_error, 'HTTP 429\nRate limited.')

    @patch('agentpulse.cache.fetch_usage', return_value={'error': 'HTTP 500'})
...
class TestRefreshingFlag(unittest.TestCase):
    ...
    def test_refreshing_true_during_api_call(self):
        ...
        with patch('agentpulse.cache.fetch_usage', side_effect=capture):
            cache.update()
...
class TestFailedTokenGuard(unittest.TestCase):
    ...
    @patch('agentpulse.cache.read_access_token', return_value='same-token')
    @patch('agentpulse.cache.fetch_usage', return_value=_AUTH_ERROR_DATA)
    ...
    @patch('agentpulse.cache.read_access_token', return_value='token-123')
    @patch('agentpulse.cache.fetch_usage', return_value=_AUTH_ERROR_DATA)
    @patch('agentpulse.cache.refresh_token')
...
class TestRateLimitGuard(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={'error': 'HTTP 429', 'rate_limited': True})
    @patch('agentpulse.cache.time')
    ...
    @patch('agentpulse.cache.fetch_usage')
    @patch('agentpulse.cache.time')
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={'error': 'HTTP 429', 'rate_limited': True, 'retry_after': 300})
    @patch('agentpulse.cache.time')
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={'error': 'HTTP 429', 'rate_limited': True, 'retry_after': 86400})
    @patch('agentpulse.cache.time')
...
class TestTokenRefresh(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_usage')
    @patch('agentpulse.cache.read_access_token', side_effect=['old-token', 'new-token'])
    @patch('agentpulse.cache.refresh_token')
...
class TestEnsureProfile(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_profile', return_value={'name': 'Test User'})
    ...
    @patch('agentpulse.cache.read_access_token', return_value='token-x')
    @patch('agentpulse.cache.fetch_profile')
...
class TestNullQuotaFields(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={'five_hour': None, 'seven_day': None})
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={'five_hour': None, 'seven_day': {'utilization': 30.0}})
    ...
    @patch('agentpulse.cache.fetch_usage', return_value={})
...
class TestEnsureProfileTokenChange(unittest.TestCase):
    ...
    @patch('agentpulse.cache.fetch_profile', return_value={'account': {'uuid': 'uuid-1'}})
    @patch('agentpulse.cache.read_access_token', return_value='token-a')
    ...
    @patch('agentpulse.cache.fetch_profile')
    @patch('agentpulse.cache.read_access_token')
