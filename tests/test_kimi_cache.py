...
from agentpulse.kimi_cache import KimiCache, KimiSnapshot
...
class TestKimiCacheInitialState(unittest.TestCase):
    def setUp(self):
        self.cache = KimiCache()
...
class TestKimiCacheUpdate(unittest.TestCase):
    def setUp(self):
        self.cache = KimiCache()

    def test_successful_update_populates_usage(self):
        usage = _make_usage(pct_5h=42.0)
        with patch('agentpulse.kimi_cache.fetch_usage', return_value=usage):
    ...
    def test_error_response_sets_last_error(self):
        with patch('agentpulse.kimi_cache.fetch_usage', return_value={'error': 'oops'}):
            result = self.cache.update()
    ...
    def test_second_update_skipped_within_cooldown(self):
        usage = _make_usage()
        with patch('agentpulse.kimi_cache.fetch_usage', return_value=usage) as mock_fetch:
            self.cache.update()
            result = self.cache.update()
    ...
    def test_lock_held_returns_none(self):
        ...
        def run_first():
            with patch('agentpulse.kimi_cache.fetch_usage', side_effect=slow_fetch):
    ...
    def test_rate_limit_sets_backoff(self):
        with patch('agentpulse.kimi_cache.fetch_usage',
    ...
    def test_version_increments_on_success(self):
        ...
        with patch('agentpulse.kimi_cache.fetch_usage', return_value=_make_usage()):
            self.cache.update()
    ...
    def test_snapshot_is_immutable_after_update(self):
        ...
        with patch('agentpulse.kimi_cache.fetch_usage', return_value=new_usage):
            with patch('agentpulse.kimi_cache.POLL_FAST', 0):
                self.cache.update()

        self.assertEqual(snap.usage['five_hour']['utilization'], 20.0)


class TestKimiCacheEnsureProfile(unittest.TestCase):
    def setUp(self):
        self.cache = KimiCache()

    def test_fetches_profile_when_none(self):
        profile = {'account': {'email': 'a@b.com'}, 'organization': {'organization_type': 'Allegretto'}}
        with patch('agentpulse.kimi_cache.fetch_profile', return_value=profile):
            with patch('agentpulse.kimi_cache.read_access_token', return_value='tok'):
    ...
    def test_does_not_refetch_when_token_unchanged(self):
        ...
        with patch('agentpulse.kimi_cache.fetch_profile', return_value=profile) as mock_fp:
            with patch('agentpulse.kimi_cache.read_access_token', return_value='tok'):
                self.cache.ensure_profile()
                self.cache.ensure_profile()
    ...
    def test_refetches_when_token_changes(self):
        ...
        with patch('agentpulse.kimi_cache.fetch_profile', side_effect=[profile1, profile2]):
            with patch('agentpulse.kimi_cache.read_access_token', side_effect=tokens):
                self.cache.ensure_profile()
                self.cache.ensure_profile()
        self.assertEqual(self.cache.profile, profile2)


class TestKimiSnapshotFrozen(unittest.TestCase):
    def test_snapshot_is_frozen_dataclass(self):
        snap = KimiSnapshot(usage={}, profile=None, last_success_time=None, refreshing=False, last_error=None, version=0)
