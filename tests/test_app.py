...
class TestInitialization(unittest.TestCase):
    """Tests for application startup provider selection."""

    def test_codex_cache_hidden_without_token(self):
        """Codex provider is not created when no Codex CLI token exists."""
        with patch('agentpulse.app.pystray'), \
             patch('agentpulse.app.create_icon_image'), \
             patch('agentpulse.app.taskbar_uses_light_theme', return_value=False), \
             patch('agentpulse.app.read_codex_access_token', return_value=None):
    ...
    def test_codex_cache_created_with_token(self):
        ...
        self.assertIsNotNone(app.codex_cache)

    def test_open_dashboard_uses_dashboard_server(self):
        """Tray action opens the local dashboard."""
        app = _make_app()
        try:
            app.dashboard = MagicMock()
            app.on_open_dashboard()
            app.dashboard.open.assert_called_once()
        finally:
            _cleanup(app)
    ...
    def test_notification_deferred_during_quiet_hours(self):
        """Quiet hours defer notifications even when the user is active."""
        app = _make_app()
        try:
            with patch.object(app, '_is_user_away', return_value=False), \
                 patch('agentpulse.app._is_quiet_time', return_value=True):
                app._notify_or_defer('threshold_five_hour', 'message', 'title')

            app.icon.notify.assert_not_called()
            self.assertIn('threshold_five_hour', app._deferred_notifications)
        finally:
            _cleanup(app)
...
class TestCheckThresholdAlerts(unittest.TestCase):
    ...
    def test_codex_threshold_uses_provider_state_key(self):
        ...
        self.assertNotIn('five_hour', self.app._notified_thresholds)
...
class TestTimeAwareAlerts(unittest.TestCase):
    ...
    def test_alert_shown_when_usage_ahead_of_time(self):
        ...
        with patch('agentpulse.app.elapsed_pct', return_value=50.0):
            self.app._check_threshold_alerts({'five_hour': {'utilization': 82, 'resets_at': '2025-01-15T14:30:00+00:00'}})
    ...
    def test_fallback_when_elapsed_pct_none(self):
        ...
        with patch('agentpulse.app.elapsed_pct', return_value=None):
            self.app._check_threshold_alerts({'five_hour': {'utilization': 82}})
    ...
    def test_usage_equal_to_time_suppressed(self):
        ...
        with patch('agentpulse.app.elapsed_pct', return_value=82.0):
            self.app._check_threshold_alerts({'five_hour': {'utilization': 82, 'resets_at': '2025-01-15T14:30:00+00:00'}})
...
class TestExtraUsageAlerts(unittest.TestCase):
    ...
    def test_notification_includes_credit_amounts(self):
        ...
        with patch('agentpulse.app.format_credits', side_effect=lambda c: f'${c / 100:.2f}'):
            self.app._check_extra_usage_alerts(self._extra_data(used=820, limit=1000))
...
class TestUpdateOrchestration(unittest.TestCase):
    ...
    def test_update_tracks_previous_values(self, _icon, _tooltip):
        ...
        self.app.dashboard.history.record.assert_called_with('claude', data)
...
class TestRenderTray(unittest.TestCase):
    ...
    @patch('agentpulse.app.format_tooltip', return_value='Usage: 42%')
    @patch('agentpulse.app.create_icon_image')
    ...
    @patch('agentpulse.app.format_tooltip', return_value='Auth Error')
    @patch('agentpulse.app.create_status_image')
...
class TestOnThemeChanged(unittest.TestCase):
    ...
    @patch('agentpulse.app.format_tooltip', return_value='tooltip')
    @patch('agentpulse.app.create_icon_image')
    @patch('agentpulse.app.taskbar_uses_light_theme', return_value=True)
...
class TestMenuActions(unittest.TestCase):
    ...
    def test_on_show_popup_guards_against_double_open(self):
        ...
        with patch('agentpulse.app.threading.Thread') as mock_thread:
...
class TestIsUserAway(unittest.TestCase):
    ...
    @patch('agentpulse.app.is_workstation_locked', return_value=False)
    @patch('agentpulse.app.get_idle_seconds', return_value=400.0)
    ...
    @patch('agentpulse.app.is_workstation_locked', return_value=False)
    @patch('agentpulse.app.get_idle_seconds', return_value=200.0)
    @patch('agentpulse.app.IDLE_PAUSE', 300)
    ...
    @patch('agentpulse.app.is_workstation_locked', return_value=False)
    @patch('agentpulse.app.get_idle_seconds', return_value=300.0)
    @patch('agentpulse.app.IDLE_PAUSE', 300)
    ...
    @patch('agentpulse.app.is_workstation_locked', return_value=False)
    @patch('agentpulse.app.get_idle_seconds', return_value=9999.0)
    @patch('agentpulse.app.IDLE_PAUSE', 0)
    ...
    @patch('agentpulse.app.is_workstation_locked', return_value=False)
    @patch('agentpulse.app.get_idle_seconds', return_value=0.0)
    @patch('agentpulse.app.IDLE_PAUSE', 300)
